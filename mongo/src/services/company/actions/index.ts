...
export const insertManyCompany = async (data: CreateManyCompanies) => {
  const companyCollection = await inCollection("company");
  const document = await companyCollection.insertMany(data);
  return document;
};

export const updateManyCompany = async (isoCode: string, data: CreateCompany) => {
  const companyCollection = await inCollection("company");
...
export const findOneCompany = async (companyId: string) => {
  const companyCollection = await inCollection("company");
